...
function uploadAssetS3Route(fastify: FastifyInstance) {
    ...
    fastify.post<{ Body: { eventId: string, type: string, description?: string } }>('/upload', { preHandler: upload.single("image") }, async (req, reply) => {
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (error.message === 'Invalid eventId') {
                    return reply.code(400).send({ error: 'Invalid eventId. Event not found.' });
                }
                if (error.message === 'Unable to create asset') {
                    return reply.code(500).send({ error: 'Unable to create asset in the database' });
                }
